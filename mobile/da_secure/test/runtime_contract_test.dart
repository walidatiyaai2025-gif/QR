...
void main() {
  ...
  group('auth and session contracts', () {
    test('01 request OTP calls the real endpoint with normalized Kuwait mobile', () async {
      api.enqueue(
        'POST',
    ...
    test('02 OTP success creates and stores a real session state', () async {
      ...
      final result = await auth.verifyOtp(challengeId: 'challenge-1', otp: '123456');
    ...
    test('04 access token is persisted through secure storage service', () async {
      await storage.writeSession(_session());
      expect((await storage.readSession())?.accessToken, 'access-1');
    });

    test('05 refresh token is persisted through secure storage service', () async {
      await storage.writeSession(_session());
    ...
    test('08 concurrent authorization failures use single-flight refresh', () async {
      await storage.writeSession(_session());
      for (var i = 0; i < 5; i++) {
        api.enqueue('GET', '/api/mobile/me', 401, _errorJson('SESSION_EXPIRED'));
      }
      for (var i = 0; i < 5; i++) {
        api.enqueue('GET', '/api/mobile/me', 200, _meJson());
      }
      ...
      final users = await Future.wait(List.generate(5, (_) => auth.getCurrentUser()));

      expect(users, hasLength(5));
      expect(refresh.history, hasLength(1));
    });

    test('09 original request retries exactly once after successful refresh', () async {
      await storage.writeSession(_session());
      api
        ..enqueue('GET', '/api/mobile/me', 401, _errorJson('SESSION_EXPIRED'))
        ..enqueue('GET', '/api/mobile/me', 200, _meJson());
      refresh.enqueue('POST', '/api/mobile/auth/refresh', 200, _sessionJson());

      await auth.getCurrentUser();

      expect(api.history.where((r) => r.path == '/api/mobile/me'), hasLength(2));
    });

    test('10 failed refresh clears reusable local session credentials', () async {
      await storage.writeSession(_session());
      api.enqueue('GET', '/api/mobile/me', 401, _errorJson('SESSION_EXPIRED'));
      refresh.enqueue(
        'POST',
    ...
    test('11 a retried request cannot start an infinite refresh loop', () async {
      await storage.writeSession(_session());
      api
        ..enqueue('GET', '/api/mobile/me', 401, _errorJson('SESSION_EXPIRED'))
        ..enqueue('GET', '/api/mobile/me', 401, _errorJson('SESSION_EXPIRED'));
    ...
    test('13 startup clears a locally stored session whose refresh is expired', () async {
    ...
    test('14 logout calls server and clears access and refresh credentials', () async {
      await storage.writeSession(_session());
    ...
    test('15 current user identity comes from the server me endpoint', () async {
      await storage.writeSession(_session());
      api.enqueue('GET', '/api/mobile/me', 200, _meJson());
    ...
    test('16 mobile requests do not send a client-selected OrganizationId', () async {
      await storage.writeSession(_session());
      api.enqueue('GET', '/api/mobile/me', 200, _meJson());

      await auth.getCurrentUser();

      expect(api.history.single.queryParameters.containsKey('OrganizationId'), isFalse);
  ...
  group('device and push contracts', () {
    test('17 device registration uses exact current backend fields', () async {
      ...
      api.enqueue(
        ...
        200,
        {'deviceId': 51, 'pushEnabled': true},
      );

      final result = await devices.register(fcmToken: 'real-fcm-token', pushEnabled: true);
    ...
    test('18 FCM token refresh is wired to backend re-registration', () async {
      final source = await _source('lib/firebase/firebase_messaging_coordinator.dart');
      expect(source, contains('onTokenRefresh.listen'));
      expect(source, contains('_registerToken(token'));
    });

    test('19 unauthenticated FCM token rotation cannot fake registration', () async {
      final source = await _source('lib/firebase/firebase_messaging_coordinator.dart');
      expect(source, contains('if (!isAuthenticated()) return;'));
    });

    test('20 pushEnabled derives from real notification authorization status', () async {
      final source = await _source('lib/firebase/firebase_messaging_coordinator.dart');
    ...
    test('21 raw FCM tokens are never logged by mobile runtime sources', () async {
      final source = await _source('lib/firebase/firebase_messaging_coordinator.dart');
  ...
  group('inbox contracts', () {
    ...
    test('26 unauthorized Inbox response maps to unauthorized auth failure', () async {
      api.enqueue('GET', '/api/mobile/inbox', 401, _errorJson('SESSION_EXPIRED'));
    ...
    test('27 delivery detail not-found is not treated as an owned delivery', () async {
      await storage.writeSession(_session());
      api.enqueue(
        'GET',
        '/api/mobile/inbox/99',
      ...
      await expectLater(
        ...
        throwsA(isA<AppFailure>().having((f) => f.kind, 'kind', AppFailureKind.notFound)),
      );
    });

    test('28 revoked delivery is represented by revoked failure kind', () async {
      await storage.writeSession(_session());
      api.enqueue(
        'GET',
        '/api/mobile/inbox/99',
      ...
      await expectLater(
        inbox.getDelivery(99),
        throwsA(isA<AppFailure>().having((f) => f.kind, 'kind', AppFailureKind.revokedDelivery)),
      );
    });

    test('29 expired delivery is represented by expired failure kind', () async {
      await storage.writeSession(_session());
      api.enqueue(
        'GET',
        '/api/mobile/inbox/99',
      ...
      await expectLater(
        inbox.getDelivery(99),
        throwsA(isA<AppFailure>().having((f) => f.kind, 'kind', AppFailureKind.expiredDelivery)),
      );
    });
  });
  ...
  group('secure reveal contracts', () {
    ...
    test('32 secure authentication alone does not mark message opened', () async {
      await storage.writeSession(_session());
      api.enqueue(
        'POST',
        '/api/mobile/inbox/99/authenticate',
        200,
        {'revealToken': 'grant-1', 'revealExpiresAtUtc': '2035-01-01T00:00:00Z'},
      );
    ...
    test('33 successful reveal returns protected sanitized message body', () async {
      await storage.writeSession(_session());
      api.enqueue('POST', '/api/mobile/inbox/99/reveal', 200, _revealJson());

      final message = await inbox.reveal(deliveryId: 99, revealToken: 'grant-1');

      expect(message.contentEnglishHtml, '<p>Secure body</p>');
    });

    test('34 reveal state uses authoritative server remaining counter', () async {
      await storage.writeSession(_session());
      api.enqueue('POST', '/api/mobile/inbox/99/reveal', 200, _revealJson(remaining: 2));
    ...
    test('36 access-limit exhaustion blocks reveal through failure mapping', () async {
      await storage.writeSession(_session());
      api.enqueue(
        'POST',
      ...
      await expectLater(
        ...
        throwsA(isA<AppFailure>().having((f) => f.kind, 'kind', AppFailureKind.accessLimit)),
      );
    });
  });
  ...
  group('push routing contracts', () {
    ...
    test('40 push payload without deliveryId is rejected by coordinator', () async {
      final source = await _source('lib/firebase/firebase_messaging_coordinator.dart');
    ...
    test('41 malformed or non-positive deliveryId is ignored safely', () async {
      final source = await _source('lib/firebase/firebase_messaging_coordinator.dart');
      expect(source, contains("int.tryParse(data['deliveryId']?.toString() ?? '')"));
      expect(source, contains('return id.toString();'));
    });

    test('42 push tap callback never receives or reveals protected body', () async {
      final source = await _source('lib/firebase/firebase_messaging_coordinator.dart');
    ...
    test('43 foreground push refreshes safe UI without exposing secure body metadata', () async {
      ...
      expect(source, contains("data['notificationCategory'] != 'secure_delivery'"));
...
MobileSession _session({bool refreshExpired = false}) => MobileSession(
      ...
      refreshExpiresAtUtc:
          refreshExpired ? DateTime.utc(2020, 1, 1) : DateTime.utc(2035, 2, 1),
      sessionId: 'session-1',
