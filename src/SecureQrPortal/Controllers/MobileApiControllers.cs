...
public sealed class MobileInboxController(MobileDeliveryAccessService deliveries) : ControllerBase
{
    ...
    private IActionResult DeliveryError(MobileDeliveryAccessStatus status) => status switch
    {
        ...
        MobileDeliveryAccessStatus.RevealBlocked => StatusCode(StatusCodes.Status503ServiceUnavailable,
            Error("SECURE_MESSAGE_REVEAL_BLOCKED", "فتح الرسائل المشفرة متوقف مؤقتًا من إعدادات النظام.", "Secure Message reveal is temporarily blocked by system settings.")),
        MobileDeliveryAccessStatus.EncryptionUnavailable => StatusCode(StatusCodes.Status503ServiceUnavailable,
            Error("SECURE_MESSAGE_CRYPTO_UNAVAILABLE", "تعذر فتح الرسالة المشفرة بأمان حاليًا.", "The encrypted Secure Message is safely unavailable at this time.")),
